...
# Read all terminology FHIR JSON resource files and combine them to one FHIR Bundle
echo "${PREFIX}Process terminology FHIR resources and create transaction FHIR Bundle ${BUNDLE}"
grep -l -E "\"resourceType\": \"(CodeSystem|ValueSet)\"" ${PWD}/fsh-generated/resources/*.json | xargs cat | \
  jq '{ resource: (.), request: { method: "POST", url: .resourceType } }' | \
    jq -s '{ resourceType: "Bundle", type: "transaction", entry: . }' > ${BUNDLE}

# Publish the created Bundle
BASE_URL=${1:-https://terminology-fgdh.wiwi.tu-dresden.de}
SERVER_ID=${2:-home}
